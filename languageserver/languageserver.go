...
func RunWithStdio(enableFlowClient bool) {
	...
	_, options, err := integration.NewFlowIntegration(languageServer, enableFlowClient)
	if err != nil {
		panic(err)
	}

	// Apply server options from integration
	err = languageServer.SetOptions(options...)
	if err != nil {
		panic(err)
	}
