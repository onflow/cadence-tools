...
func (l standardLibrary) ValidatePublicKey(_ *stdlib.PublicKey) error {
	// Implementation should never be called,
	// only its definition is used for type-checking
	panic(errors.NewUnreachableError())
}

func (l standardLibrary) BLSAggregatePublicKeys(_ []*stdlib.PublicKey) (*stdlib.PublicKey, error) {
	// Implementation should never be called,
	// only its definition is used for type-checking
	panic(errors.NewUnreachableError())
}

func (l standardLibrary) BLSAggregateSignatures(_ [][]byte) ([]byte, error) {
	// Implementation should never be called,
	// only its definition is used for type-checking
	panic(errors.NewUnreachableError())
}

func (l standardLibrary) VerifySignature(
	_ []byte,
	_ string,
	_ []byte,
	_ []byte,
...
func (l standardLibrary) BLSVerifyPOP(_ *stdlib.PublicKey, _ []byte) (bool, error) {
	// Implementation should never be called,
	// only its definition is used for type-checking
	panic(errors.NewUnreachableError())
...
func (l standardLibrary) Hash(_ []byte, _ string, _ sema.HashAlgorithm) ([]byte, error) {
	// Implementation should never be called,
	// only its definition is used for type-checking
	panic(errors.NewUnreachableError())
...
func (l standardLibrary) AccountKeysCount(_ common.Address) (uint64, error) {
	// Implementation should never be called,
	// only its definition is used for type-checking
	panic(errors.NewUnreachableError())
...
func newStandardLibrary() (result standardLibrary) {
	...
	for _, valueDeclaration := range stdlib.DefaultScriptStandardLibraryValues(result) {
